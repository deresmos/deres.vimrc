...
impl LazyMaker {
    ...
    pub fn make(&self, output_path: String) -> Result<()> {
        ...
        let plugins: Vec<LazyPlugin> = plugin_paths
            .iter()
            .filter_map(|path| LazyPlugin::from_path(path.into()))
            .collect();
        ...
        handlebars
            .register_template_file("template", "./src/plugin_manager/lazy.hbs")
            .unwrap();
        let mut output_file = File::create(output_path)?;
        handlebars.render_to_write("template", &data, &mut output_file)?;
        // let dein_toml = LazyToml::new(&plugins);
